  ...
  status_interfaces__msg__SystemStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    status_interfaces__msg__SystemStatus * data =
      (status_interfaces__msg__SystemStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!status_interfaces__msg__SystemStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          status_interfaces__msg__SystemStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
